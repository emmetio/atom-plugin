...
const suffixMarkersCache = new WeakMap();
...
function getExpandedAbbreviationCompletion({editor, bufferPosition, prefix, activatedManually}) {
	...
	if (marker) {
		...
		let suffixMarker;
		if (abbrSuffix) {
			// Add marker at the end of extracted abbreviation. If current snippet
			// will be expanded, this marker will be used to remove suffix afterwards
			const suffixRange = [bufferPosition, bufferPosition.traverse([0, abbrSuffix.length])];
			suffixMarker = editor.markBufferRange(suffixRange);
			suffixMarkersCache.set(editor, suffixMarker.id);
		}
...
function disposeSuffixMarkers(editor) {
	const marker = editor.getMarker(suffixMarkersCache.get(editor));
	if (marker) {
		marker.destroy();
	}
	suffixMarkersCache.delete(editor);
}
