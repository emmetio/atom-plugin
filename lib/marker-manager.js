...
export default class MarkerManager {
	...
	markAbbreviationForKnownSyntax() {
		const syntax = detectSyntax(this.editor);
		if (this.editor && syntax) {
			if ((syntax === 'js' || syntax === 'jsx') && !this.editor.bufferRangeForScopeAtCursor('JSXNested')) {
				// aut-insert marker into explicit JSX context only
				return;
			}
