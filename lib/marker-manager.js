...
export default class MarkerManager {
	...
	setCurrentEditor(editor) {
		...
		if (typeof buffer.onDidChangeText === 'function') {
			this.editorSubscriptions.add(buffer.onDidChangeText(this.markAbbreviation));
		} else {
			// TODO: Remove this after `TextBuffer.prototype.onDidChangeText` lands on Atom stable.
			this.editorSubscriptions.add(buffer.onDidChange(this.markAbbreviation));
		}
	}
	 ...
	 */
	markAbbreviation() {
		if (this.editor && !hasExpansions(this.editor)) {
	...
	isSupportedEditor(editor) {
		// TODO detect supported syntaxes
