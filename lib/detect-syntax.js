...
/**
 * Contains syntax-specific checks to decide whether we should provide automatic
 * abbreviation marker for current editor context
 * @type {Object}
 */
const autoActivationContext = {
	jsx(editor) {
		return !!editor.bufferRangeForScopeAtCursor('JSXNested');
	},
	html(editor) {
		// Do not provide automatic abbreviation completoin inside HTML tags
		return !editor.bufferRangeForScopeAtCursor('meta.tag.inline');
	}
};
...
export function hasAutoActivateContext(editor) {
	...
	return !autoActivationContext[syntax] || autoActivationContext[syntax](editor);
}
